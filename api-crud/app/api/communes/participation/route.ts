...
export async function POST(request: NextRequest) {
  try {
    ...
    const existingParticipation = await prisma.participationCommune.findFirst({
      where: { codeCommune: parseInt(codeCommune) }
    });
    ...
    const result = await prisma.participationCommune.create({
      data: {
        codeCommune: parseInt(codeCommune),
        nombreBureaux: nombreBureaux ? parseInt(nombreBureaux) : null,
        nombreInscrits: inscrits || null,
        nombreVotants: votants || null,
        tauxParticipation: tauxPart || null,
        bulletinsNuls: nuls || null,
        suffragesValables: valables || null,
        tauxAbstention: tauxAbst || null,
      }
    });
...
export async function GET(request: NextRequest) {
  try {
    ...
    const participation = await prisma.participationCommune.findFirst({
      where: { codeCommune: parseInt(codeCommune) },
      include: {
        commune: true
      }
